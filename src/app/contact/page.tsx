  ...
  return (
    <main className="bg-[var(--color-background)] text-[var(--color-secondary)] mb-8 md:mb-20 mx-4 md:mx-8 mt-24 md:mt-28">
      <section className="max-w-6xl mx-auto text-center mb-16">
        <h1 ref={titleRef} className="text-4xl md:text-6xl font-bold mb-6 text-[var(--color-primary)] font-primary leading-tight flex justify-center" aria-label="Contact">
      ...
      <div className="md:flex md:space-x-16 max-w-6xl mx-auto">
        <section className="md:w-1/2 text-center md:text-left">
          <div className="space-y-4 mb-8">
            <button
              className="text-xl py-3 px-8 rounded-md cursor-pointer transition-colors font-helvetica-neue duration-300 hover:bg-blue-700 relative overflow-hidden"
            ...
            >
              ...
              <span ref={cta1HoverTextRef} className="absolute inset-0 flex items-center justify-center translate-y-full opacity-0" aria-hidden="true">Book a Free Consult</span>
            </button>
            <p className="text-lg font-helvetica-neue">
              ...
              >
                ...
                <span ref={cta2HoverTextRef} className="absolute inset-0 flex items-center justify-center translate-y-full opacity-0" aria-hidden="true">Email Us</span>
              </a>
            </p>
          </div>
        ...
        <section className="md:w-1/2 text-center md:text-left">
          <h2 className="text-3xl font-bold mb-4 text-[var(--color-primary)] font-primary">Stay in the Loop</h2>
          <p className="text-lg font-helvetica-neue mb-8">Subscribe to our newsletter for the latest AI insights and updates.</p>
          <form onSubmit={handleSubmit} className="flex flex-col md:flex-row items-center md:space-x-2">
            ...
            <input 
              ...
              className={`bg-transparent border rounded-md py-2 md:py-4 px-4 text-lg font-helvetica-neue focus:outline-none focus:ring-2 w-full mb-2 md:mb-0 ${emailError ? 'border-red-500 focus:ring-red-500' : 'border-[var(--color-primary)] focus:ring-[var(--color-primary)]'}`}
              value={email}
              onChange={(e) => {
            ...
            <button 
              type="submit" 
              className="text-lg py-6 md:py-7 px-10 rounded-md cursor-pointer font-helvetica-neue transition-colors duration-300 border border-white relative overflow-hidden w-full md:w-auto"
              onMouseEnter={() => handleMouseEnter(cta3TextRef, cta3HoverTextRef)}
              onMouseLeave={() => handleMouseLeave(cta3TextRef, cta3HoverTextRef)}
            >
              <span ref={cta3TextRef} className="absolute inset-0 flex items-center justify-center">Sign Up</span>
              <span ref={cta3HoverTextRef} className="absolute inset-0 flex items-center justify-center translate-y-full opacity-0" aria-hidden="true">Sign Up</span>
            </button>
          </form>
